...
#ifdef _ATLAS_SHADOW
#define DECLARE_ATLAS const in vec4 shadowMapSize,
#else
#define DECLARE_ATLAS
#endif

#ifdef _NORMAL_OFFSET
#define DECLARE_NORMAL_OFFSET ,const in float normalBias
#else
#define DECLARE_NORMAL_OFFSET
#endif

#ifdef _OUT_DISTANCE
#define DECLARE_OUT_DISTANCE ,out float outDistance
#else
#define DECLARE_OUT_DISTANCE
#endif
...
float computeShadow(const in bool lighted,
                    ...
                    const in float bias
                    DECLARE_NORMAL_OFFSET
                    DECLARE_OUT_DISTANCE) {
    ...
    if (earlyOut) {
        ...
        float shadowBias = 0.05 * sqrt( 1.0 - N_Dot_L * N_Dot_L) / clamp(N_Dot_L, 0.0005, 1.0);
        ...
        shadowReceiverZ = clamp(shadowReceiverZ, 0.0, 1.0 -shadowBias) - shadowBias;

        // Now computes Shadow
#ifdef _OUT_DISTANCE
        outDistance = shadowReceiverZ - getSingleFloatFromTex(tex, clamp(shadowUV + shadowBiasPCF, clampDimension.xy, clampDimension.zw));
        outDistance *= depthRange.z; // world space distance
        if (lighted) shadow = getShadowPCF(tex, shadowTextureSize, shadowUV, shadowReceiverZ, shadowBiasPCF, clampDimension);
#else
